...
Product-fit candidates require active OHLQ status (70), an active non-discontinued tenant portfolio decision, and a current statewide store-distribution listing. A3A DIST / distillery-only and delisted items are excluded. Saved scores are rechecked at display time, so eligibility changes do not wait for the next scoring run. A listing elsewhere in Ohio can support an expansion recommendation even if this agency does not yet carry the product; zero on-hand does not disqualify a listed product. This filter does not remove existing inventory or sales history.
