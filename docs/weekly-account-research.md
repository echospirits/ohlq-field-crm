...
- It selects at most 250 due accounts with complete street, city, and ZIP identity from the current tenant's opportunities; incomplete locations stay in the manual CSV queue.
- It submits only one manually advanced wave of 25 accounts at a time. Another wave cannot start while submitted jobs are still running.
- Each account reserves $0.08 before submission, so the run can never reserve more than the $20 application ceiling.
...
Every completed result is automatically checked against these rules. Exact matches upsert the account's shared public facts and immediately recalculate that tenant's affected opportunity. Uncertain or changed identities are declined and change no account or opportunity data. The job ledger preserves the evidence, validation result, cost, and decision.
