...
import { getOrganizationTenantConfig, matchesTenantProduct } from '../../../lib/tenantConfig';
import type { OutcomeModel } from '../../../lib/opportunityLearning';
...
export default async function OpportunityPerformancePage() {
  ...
  const [opportunities, visits, followUps, sales, latestModel] = await Promise.all([
    ...
    prisma.accountSalesEvent.findMany({ where: { organizationId, sourceKey: { startsWith: 'DAILY_V3:' } }, select: { wholesaleAccountId: true, reportDate: true, vendor: true, itemCode: true } }),
    prisma.opportunityModelVersion.findFirst({ where: { name: `${organizationId}:PriceAffinityRanker` }, orderBy: { createdAt: 'desc' }, select: { evaluation: true, createdAt: true } }),
  ]);
  const purchases = sales.filter(p => matchesTenantProduct({ config, vendor: p.vendor, itemCode: p.itemCode }));
  const learning = latestModel?.evaluation as unknown as OutcomeModel | null;
  ...
  return <><header className="page-heading"><span className="page-eyebrow">Learning</span><h1>Opportunity performance</h1><p className="muted">Observed relationships, not claims that CRM activity caused a purchase.</p></header>
    <section className="card"><h2>{learning?.active ? 'Learning is influencing scores' : 'Collecting evidence for learning'}</h2><p>Product and price fit update with daily imports. Existing-buyer comparisons use this organization’s products. Outcome learning waits for complete 90-day purchase windows and must outperform a baseline on later accounts before it can adjust scores.</p><p>{learning ? `${learning.trainingCount} training accounts, ${learning.holdoutCount} validation accounts, ${learning.conversions} training conversions.` : 'The revised engine has not run for this organization yet.'}</p><p className="muted">Historical results from the earlier scoring system remain below. Purchase follow-up statistics use the corrected daily ledger and do not count legacy delta records twice.</p></section>
    ...
    <div className="grid"><RelationshipCard title={`Visits followed by a ${config.productLabel} purchase`} results={visitResults}/><RelationshipCard title={`Completed follow-ups followed by a ${config.productLabel} purchase`} results={followUpResults}/></div></>;
}
