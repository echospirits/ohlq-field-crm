...
async function addVendor(formData: FormData) {
  ...
  const candidates = await prisma.ohlqAgencyInventoryCurrent.findMany({ where: { vendorId }, distinct: ['itemCode'], orderBy: { itemCode: 'asc' }, select: { itemCode: true, itemName: true } });
  await prisma.organizationProduct.createMany({ skipDuplicates: true, data: candidates.map((item) => ({ organizationId, market: 'OH', externalItemCode: item.itemCode, displayName: item.itemName, status: OrganizationProductStatus.PENDING_REVIEW })) });
...
async function setPendingProductStatus(formData: FormData) {
  ...
  const organizationId = clean(formData.get('organizationId'));
  const status = clean(formData.get('status')) as OrganizationProductStatus;
  if (status !== OrganizationProductStatus.OWNED && status !== OrganizationProductStatus.EXCLUDED && status !== OrganizationProductStatus.REPRESENTED) return;
