...
async function saveFeatures(formData: FormData) {
  ...
  const enabled = assertFeatureDependencies(getPackageFeatureKeys(intelligenceEnabled, directWholesaleOrdersEnabled, analyticsEnabled));
  ...
  await writeOrganizationAudit(actor.id, id, OrganizationAuditAction.FEATURE_CHANGED, { coreIncluded: true, directWholesaleOrdersEnabled, intelligenceEnabled, analyticsEnabled, enabled });
  revalidatePath(`/platform/organizations/${id}`);
...
export default async function OrganizationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams?: Promise<{ status?: string }> }) {
  ...
  const analyticsEnabled = enabled.has('ANALYTICS');
