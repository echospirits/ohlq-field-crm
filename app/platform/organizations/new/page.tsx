...
async function provisionOrganization(formData: FormData) {
  ...
  const a3aStoreIds = normalizeOrganizationIdentifierList(clean(formData.get('a3aStoreIds')));
  ...
  const features = assertFeatureDependencies(getPackageFeatureKeys(intelligenceEnabled, directWholesaleOrdersEnabled, analyticsEnabled));
  ...
  const result = await prisma.$transaction(async (tx) => {
    const organization = await tx.organization.create({ data: {
      ...
      onboardingData: { identity: true, admin: true, ohioMarket: vendorIds.length > 0, a3aStoresConfigured: a3aStoreIds.length > 0, features: features.length > 0, productsConfirmed: false, firstLogin: false },
    } });
...
export default async function NewOrganizationPage({ searchParams }: { searchParams?: Promise<{ status?: string }> }) {
  ...
  return <>
    ...
    <form action={provisionOrganization} className="platform-provisioning">
      ...
      <fieldset className="card"><legend>3–4. Ohio data and product discovery</legend><div className="form-grid"><label>Ohio Vendor IDs<textarea name="vendorIds" placeholder="One or more IDs, separated by commas or new lines" rows={3} /></label><label>A3A Store IDs<textarea name="a3aStoreIds" placeholder="Store numbers located in this organization's locations" rows={3} /></label></div><p className="muted">Vendor IDs discover matching products and remain Platform Admin-only. Organization admins can maintain A3A Store IDs and choose which discovered products to include.</p></fieldset>
      ...
      <fieldset className="card"><legend>5. Plan</legend><p className="muted">Core CRM is always included. Enable optional capabilities explicitly.</p><div className="package-entitlement-grid"><article className="package-entitlement"><span><strong>Core CRM</strong><small>Accounts, visits, worklists, users, Ohio data, integrations, and standard reporting.</small></span><span className="pill">Included</span></article><label className="package-entitlement selectable"><input name="intelligence" type="checkbox" /><span><strong>Intelligence</strong><small>Agency Intelligence, Wholesale Opportunities, and Advanced Intelligence.</small></span><span className="pill">Add-on</span></label><label className="package-entitlement selectable"><input name="analytics" type="checkbox" /><span><strong>Analytics</strong><small>Sales, account performance, and activity reporting with CSV exports.</small></span><span className="pill">Pilot</span></label><label className="package-entitlement selectable"><input name="directWholesaleOrders" type="checkbox" /><span><strong>Direct Wholesale Orders</strong><small>Prepare and download Ohio A-3a wholesale-order PDFs.</small></span><span className="pill">Pilot</span></label></div></fieldset>
      <section className="card provisioning-review"><div><span className="page-eyebrow">6. Discover and review</span><h2>Create configuration</h2><p className="muted">This creates an isolated tenant configuration and discovers product candidates. Review ownership on the next screen, then run the persisted provisioning workflow.</p></div><SubmitButton type="submit">Create and discover products</SubmitButton></section>
    </form>
  </>;
}
