...
test('customer configuration lives on Organization and sensitive operational rows require tenant ownership', () => {
  ...
  for (const model of ['OrganizationA3aStoreIdentifier', 'OrganizationOhlqCredentials', 'OhlqAgencyInventorySnapshot', 'OhlqAgencyInventoryCurrent', 'OhlqTenantInventoryImportStatus', 'LoggedVisit', 'WorklistItem', 'LocationContact', 'MenuPlacement', 'Recipe', 'SalesOpportunity']) {
  ...
  assert.match(schema, /@@id\(\[organizationId, snapshotDate, agencyNumber, itemCode\]\)/);
  assert.match(schema, /@@id\(\[organizationId, agencyNumber, itemCode\]\)/);
});
